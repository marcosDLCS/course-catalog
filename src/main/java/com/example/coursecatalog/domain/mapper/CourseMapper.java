...
public interface CourseMapper {
    ...
    @Select("SELECT C.id as id, C.name as name, C.level as level, C.hours as hours, C.teacher as teacher, CONCAT_WS(' ', T.name, T.surname) as teacherName FROM COURSE C, TEACHER T WHERE C.active = true AND C.teacher = T.id")
    List<Course> findAll();
